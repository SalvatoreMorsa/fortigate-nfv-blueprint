...
# TODO: delete_in_relationship
@operation
def create_in_relationship(config_name, config_id, config, ssh_config, **_):
    '''Generic config create operation'''
    # Create the configuration
    iface = Config(name=config_name,
                   cid=config_id,
    ...
    ctx.source.instance.runtime_properties['config_name'] = config_name
    ctx.source.instance.runtime_properties['config_id'] = config_id
    ctx.source.instance.runtime_properties['config'] = iface.read()
    # Dump the runtime properties
    ctx.logger.debug('Runtime properties: {0}'.format(
